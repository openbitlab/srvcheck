...
install_monitor () {
    wget -q http://raw.githubusercontent.com/openbitlab/srvcheck/dev/$1.sh -O /root/$1.sh ## TODO add args to change file path
    chmod +x /root/$1.sh
...
install_service () {
    wget -q http://raw.githubusercontent.com/openbitlab/srvcheck/dev/node-monitor.service -O /etc/systemd/system/node-monitor.service ## TODO add args to change service name
