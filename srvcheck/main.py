...
from .utils import ConfItem, ConfSet, Persistent, System
...
ConfSet.addItem(ConfItem("tasks.horcruxService", None, str, "Horcrux service name"))
...
def main():  # noqa: C901
    ...
    for x, v in NOTIFICATION_SERVICES.items():
        if conf.exists(f"notification.{x}.enabled") and conf.getOrDefault(
            f"notification.{x}.enabled", False
        ):
